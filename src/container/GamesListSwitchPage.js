...
import GameListSwitchForm, { EditButton, DeleteButton } from '../component/GamesListSwitchForm';
...
@observer class GamesListSwitchPage extends React.Component {
  ...
  gameSwitchList = () => {
    const { user } = this.props.parentState;
    let xhr = new XMLHttpRequest();
    xhr.open('get',`${basename}/api/GamesByUser/${user.userId}/2`)
    xhr.responseType ='json';
    xhr.addEventListener('load',()=>{
      if(xhr.status === 200){
        let {data} = xhr.response;
        this.pageState.GameSwitchList = data.map(g =>{
  ...
  processPdf =() =>{
    ...
    this.props.showLoadingScreen(true);
    let xhr = new XMLHttpRequest();
    xhr.open('get',`${basename}/api/pdfswitch`)
    xhr.responseType ='json';
    xhr.addEventListener('load',()=>{
      this.props.showLoadingScreen(false);
      if(xhr.status === 200){
        saveAs(`${basename}/api/pdfswitch`);
...
export default GamesListSwitchPage;